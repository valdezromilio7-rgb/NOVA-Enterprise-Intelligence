"""Reproducible Account Intelligence E2E evaluation with canonical evidence."""
...
from factory.account_intelligence.evidence import Evidence, EvidenceVerificationState, build_evidence
from factory.account_intelligence.evidence_opportunity import business_problem_to_opportunity_with_evidence
...
class AccountIntelligenceEvidenceE2EResult:
...
def run_end_to_end_evaluation_with_evidence(
    fixture: AccountIntelligenceFixture | None = None,
    top_n: int = 20,
) -> AccountIntelligenceEvidenceE2EResult:
    """Execute the v0.1 chain with canonical Evidence IDs downstream."""
    if top_n <= 0:
    ...
    scores = rank_signal_scores(
        tuple(
            score_signal(
                signal,
                _observable_dimensions(observation_by_id[signal.observation_ids[0]].content),
            )
    ...
    context = build_account_context(
        ...
        facts=(observation.content,),
        evidence_refs=evidence_refs,
        current_state="Customer support receives delivery-status requests.",
        change_event="Repeated delivery-status demand is observed.",
        confidence=observation.confidence,
        provenance=observation.provenance,
    )
    why_now = assess_why_now(
        context=context,
    ...
    opportunity = business_problem_to_opportunity_with_evidence(
        problem,
        target_customer=account.name,
        evidence_ids=evidence_refs,
    )

    return AccountIntelligenceEvidenceE2EResult(
